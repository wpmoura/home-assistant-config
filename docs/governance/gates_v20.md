...
## Health Check — Timeline, frequências mensais e coerência temporal (Gates SOC, 2026-09-19/21)

**Status: HOMOLOGADO.** Três entregas da frente Health Check, todas SOC (nova fonte/evento na allowlist do contrato canônico; alteração do scheduler Node-RED; apresentação da página Saúde do Sistema).

**1. Health Check → Timeline V20.** Automação `saude_sistema_health_check_publicar_timeline_v20` em `packages/saude_sistema_timeline.yaml`, gatilho por evento `health_check_state_changed` (o sensor `sensor.saude_sistema_analitico_status` é restaurado como `unavailable→success` a cada restart e republicaria um resultado falso). Elegibilidade fail-closed: `estado` `success`/`failed`, `origem` `manual`/`scheduled`, `execution_id` não vazio e `modelo` fora de {vazio, `mock`, `none`, `null`, `unknown`, `unavailable`}. Identidade: `request_id`/`session_id` por `md5(execution_id ~ ':health_check:<código>:<request|session>')`, logo uma execução gera no máximo uma publicação (identidade lógica `source+session_id+event_code`). Eventos: `health_check_completed` ("✅ Health Check concluído") e `health_check_failed` ("❌ Health Check falhou"), publicados por `script.turn_on` (fire-and-forget) no publicador canônico. Allowlist replicada em `contrato_publicacao_timeline_v20.yaml` e `motor_timeline_v20.yaml`; a dedup por texto ganhou exceção restrita a esses dois eventos nas 10 linhas `heartbeat_excecao` (matriz de 210 combinações: só `health_check_*` mudou). SmallTV, heartbeat e `ha_started` não foram alterados. Homologação (2026-09-19): `check_config` válido, um restart do Core, execução manual real `hc-mu8v8vxy-jaj2pllz` (Haiku 4.5), ACK `published` (`request_id 91ae47c2-cee1-4c0a-8900-868a97c4bbab`, coerente com o md5 do `execution_id`), uma única entrada na Timeline, trace da automação com 4 execuções `failed_conditions` (estados intermediários) e 1 executada.

**2. Frequências mensais.** `input_select.saude_sistema_health_check_frequencia` ganhou `1x por mês` (dia 1) e `2x por mês` (dias 1 e 15), às 08:00 America/Sao_Paulo (11:00 UTC; `getUTCDate()`, coerente porque a data UTC às 11:00 coincide com a local). As cinco opções anteriores e o padrão `Desativado` não mudaram; a frequência segue `Desativado`. A regra vive em `gate53d_fn_decidir` (tab `gate53b_tab` do Node-RED), implantada por um único `PUT /flow/gate53b_tab` alterando somente o campo `func`; **a lógica do Node-RED está no runtime e não é versionada neste repositório**. SHA-256 canônico da tab (`json.dumps` com `sort_keys`, separadores compactos, `ensure_ascii=False`): baseline/rollback `45156a39c397087e745f4874c649d9aecca18c5558fd765fc7425a029f6a4ed2`; pós-PUT `bfd6ffd65fd62f0775ee58cecb1bd32f101d521819a752c4e6d7476e6ac09ba5` (igual ao esperado; 73 nós, só `func` de um nó alterado). Validação offline em Node.js (sem rede): 15.330 combinações das opções existentes sem divergência e 17 casos novos conformes; o trace ao vivo passou a registrar `diaMes`. Ativação do helper por `input_select.reload` (sem restart), após checagem de que os helpers com `initial` estavam nos valores iniciais. Nenhuma execução real numa janela mensal ocorreu ainda: a validação foi offline e por trace ao vivo do ramo não-devido; a primeira janela natural é 01/10/2026 às 08:00 e só ocorre se uma das novas opções for selecionada.

**3. Coerência temporal da página Saúde do Sistema.** Dashboard `sistema-casa` (view `saude-do-sistema`) atualizado pela API suportada do HA (`ha_config_set_dashboard` com `config_hash`); `.storage` (Lovelace Storage) não é versionado. 12 campos alterados (11 `content` e 1 `heading`); hash canônico do dashboard `d65cd6b9…` → `7c8cf489…` (= o esperado). Diagnóstico: a camada analítica é renovada a cada execução; a camada determinística (`sensor.saude_sistema_status`) é uma **publicação manual** (`source: claude_hamcp_manual`, `checked_at` 2026-09-01T01:01:25-03:00, watchdog `sem_execucao`), sem produtor automático — decisão: **não criar produtor** e sim identificá-la como publicação manual, com data e frescor. Foram corrigidos rótulos de temporalidade (Estado da Execução, Resultado, Telemetria) e textos obsoletos. Metadados: `contrato.publicado_por` e o comentário "Ainda 100% MOCK" do `input_button` em `packages/saude_sistema_analitico.yaml`. `publicado_por` já está implantado (`template.reload` em 2026-09-21) e materializa no próximo evento legítimo do Health Check (sensor trigger-based restaura atributos até lá). Homologação sem chamada Anthropic: 138 verificações offline em 7 cenários (success, em andamento, falha após sucesso, MOCK, watchdog ok/atrasado/sem_execucao) mais renderização no motor de templates do HA contra os estados atuais.

**Limitações e residuais (não bloqueantes):** a lógica do Node-RED (scheduler e demais fluxos do Health Check) não é versionada; a camada determinística continua manual e congelada na última publicação; a procedência de `last_changed` na coleta enviada à IA (reinicia no boot do HA) não foi tratada nesta frente.
