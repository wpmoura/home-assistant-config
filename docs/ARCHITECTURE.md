...
Antes da implantação foi identificado e corrigido estaticamente um bloqueador no consumidor canônico: `sensor.casa_evento_publicavel_v20` reconhecia somente os quatro códigos CSMR. O consumidor passa a validar as combinações autorizadas de `source`, `event_code` e mensagem tanto para `csmr_v20_2c` quanto para `carro_presenca`, preservando as mensagens existentes e acrescentando `car_use_started`/`car_use_ended`. Em `sensor.casa_timeline_v20`, estado do evento, `request_id`, `publicar_timeline` e demais atributos correlacionados são lidos atomicamente de `trigger.to_state`; não há combinação com o estado corrente de outra atualização. `publicar_timeline` autoriza somente quando é booleano nativo e exatamente `true`, sem conversão ou fallback permissivo. `request_ids_json` somente incorpora o request sob a mesma decisão que materializa uma mensagem válida em `eventos_json` e nas linhas visíveis; `sem_evento`, evento inválido, mensagem vazia ou deduplicação visual não confirmam publicação. Assim, o ACK `published` continua dependente da presença do request no ledger, mas esse registro passa a comprovar também a materialização. A correção foi aprovada em revisão estática independente para commit e push. A homologação runtime deverá confirmar que, em eventos consecutivos com a mesma mensagem, o segundo é deduplicado visualmente, seu `request_id` não entra no ledger e o publicador não recebe ACK `published` falso; implantação e homologação runtime permanecem suspensas.

Na primeira tentativa de implantação, o Core 2026.7.1 recusou o filtro Jinja `hash` usado pelo produtor e os sete `input_text` novos foram materializados como `unknown`. A correção estática localizada substitui o filtro incompatível pela função global `md5`, já adotada pelo CSMR e comprovada no ambiente, mantendo digest independente para sessão, início e término e formatação UUID v4 antes da persistência. A primeira materialização passa por `script.carro_inicializar_checkpoints_primeira_criacao`, ação administrativa manual, `mode: single`, protegida pelo marcador persistente `input_boolean.carro_checkpoints_inicializados`: somente entidades existentes, carro fora de uso e valores exclusivamente vazios/`unknown` podem ser convertidos para vazio. Qualquer `unavailable`, texto inválido, checkpoint ou rejeição real bloqueia o bootstrap; o marcador é gravado por último, permite retomada de limpeza parcial e impede que indisponibilidade posterior seja confundida com primeira criação. Publicação, reconciliação e liberação exigem o marcador `on`. A correção ainda aguarda revisão estática independente; não houve nova implantação ou homologação runtime.
