...
Última reconciliação: 2026-09-17
...
| PEND-022 | CSMR / presença / Timeline — falso contexto de Casa | SOC | Eventos de saída/chegada de Casa e abertura/encerramento do monitoramento incompatíveis com os locais reais informados por Wilson em 16–17/09/2026 | incidente funcional | Sim para confiar na semântica de presença e nas ações dependentes do CSMR; não reabre a homologação H4 sem evidência de defeito no seu mecanismo | ABERTA | Segundo Wilson, Casa, trabalho e casa de Fernanda são todos locais cobertos por zonas nomeadas já cadastradas no Home Assistant; não presumir ausência de cadastro nem equivalência semântica com a zona `home`. Em 16/09 às 19:00, a Timeline publicou `Wilson saiu de casa` e `Monitoramento remoto iniciado` quando Wilson saía do trabalho; às 19:54 publicou chegada em Casa, fim do uso do carro e encerramento do monitoramento, chegada confirmada por Wilson. Em 17/09 às 05:50 publicou `Wilson chegou em casa`, fim do uso do carro e do monitoramento, embora Wilson relate que acordou na casa de Fernanda; às 06:54 publicou nova saída de Casa e início do monitoramento sem saída física. Capturas do histórico de localização do iPhone às 06:52:58, 06:54:04, 06:56:22 e 06:58:07 exibem a mesma região aproximada, sem deslocamento demonstrado; o círculo de precisão não prova coordenada exata. Diagnóstico inicial somente leitura: correlacionar histórico bruto e atributos de `person.wmoura`, tracker selecionado, nomes/limites e transições das três zonas cadastradas, precisão GPS com sessões/checkpoints CSMR, publicador canônico, Timeline e eventos de uso do carro nas duas janelas; separar erro da fonte de presença, da interpretação de `home`, da reconciliação e da publicação, e identificar eventuais efeitos físicos/push. Não corrigir por inferência, não disparar novo ciclo e não associar a rajada de internet/failover às 03:34–03:35 ao GPS sem evidência |
...
| PEND-021 — V20.1C/legado / blueprint Bloqueado (monitoramento de energia crítica) | RESOLVIDA | Gate P2 delimitado implantado e validado em 2026-09-16, promovido a `main` via PR #49 (merge commit `7c4390c440f5d7c4a94b0a46f4eb06791d2f9e95`, commit de conteúdo `2f66dad92c426e8c7db6dd5f8a32cd3d094cb219`). Removidas do `automations.yaml` operacional (`/Volumes/config`, com backup sha256 conferido antes) somente as duas entradas órfãs `use_blueprint: wmoura/monitoramento_energia_v5.yaml` — `id 1777423497558` (Geladeira) e `id 1777424244998` (Cervejeira) —, caminho de blueprint inexistente em Git e na listagem live, nunca versionado neste repositório. `check_config` PASS; `automation.reload` isolado (sem restart) executado sem regressão. Pós-reload confirmado: as duas automações `v6` — `Cozinha - Gestão energia Geladeira` (`id 1777424726293`) e `Dispensa - Gestão Energia Cervejeira` (`id 1777425009523`) — permanecem `state: on`, com todos os parâmetros preservados intactos, incluindo `limite_corrente_alta: 3` da cervejeira (divergente do `5.5` que a `v5` morta tinha, mas deliberadamente mantido sem alteração — Wilson não recorda a origem do valor atual, e a calibração é aceita como questão separada e não bloqueante, sem correção nesta frente). Os dois registros órfãos remanescentes no `entity_registry` (`automation.nova_automacaogeladeira_2`, `automation.dispensa_gestao_energia_cervejeira`, ambos `unavailable`/`restored: true`) foram conscientemente aceitos como resíduo não crítico — remover entradas do YAML não limpa o `entity_registry` automaticamente, e nenhuma limpeza de registry foi autorizada ou executada nesta frente; podem ser tratados em lote de higienização futuro (mesmo padrão de PEND-005/PEND-018), sem reabrir esta pendência |
| PEND-020 — V20.1C/legado / eletrodomésticos (micro-ondas) | RESOLVIDA | Gate P2 delimitado implantado e validado em 2026-09-16. Comparação completa (sem auditoria geral): `Cozinha - Liga Microondas` (`id 1743565284745`) e `LAB - Liga Agenda Micro Ondas` (`id 1743632883172`) tinham gatilho idêntico (`schedule.agenda_micro_ondas → on`) e ação idêntica (`switch.turn_on` em `switch.micro_ondas_tomada_1`), diferindo só pela condição de guarda (switch off) de `LAB`, funcionalmente inerte por idempotência do domínio `switch` (mesmo raciocínio do commit `036e207`, fechamento Monitor Dell/PEND-011). Evidência de disparo real já confirmada antes da remoção: ambas dispararam em `2026-09-16T11:00:00` (mesmo horário); `LAB - Liga Agenda Micro Ondas` preservada sem alteração após o reload. Backup do `automations.yaml` operacional (`/Volumes/config`) criado e conferido por sha256 antes da edição; removido dele **somente** o bloco `id 1743565284745`; diff restrito confirmado (15 linhas, nenhum outro ID/parâmetro/arquivo tocado, incluindo as demais alterações locais preexistentes de `/Volumes/config`). `check_config` PASS; `automation.reload` isolado (sem restart) executado. **Pós-reload, confirmado sem presumir:** `LAB - Liga Agenda Micro Ondas` segue `state: on`, gatilho/guarda/ação idênticos aos de antes da remoção; `id 1743565284745` saiu da configuração carregada, mas seu registro **permanece no `entity_registry`** como `unavailable`/`restored: true` — não removido, não presumido limpo (mesma ressalva já aplicada em PEND-021/PEND-005/PEND-018). Nenhuma regressão observada; rollback não foi necessário. O próximo disparo natural da agenda pode ser observado depois, sem ser condição deste fechamento — a equivalência já foi comprovada estaticamente e reforçada pelo histórico de disparo real de ambas antes da remoção |
