...
| PEND-012 | V20.1C/decommission | SOC | Definir e autorizar lotes pequenos de desativação com rollback | decisão | Sim para qualquer remoção | ABERTA | Lote 1 executado (`automation.sala_porta_da_sala_abriu`; blueprint `wan_4G_AppleWatch.yaml` sem `_Energia`); Lote 2 executado (3 scripts mortos removidos de `scripts_erro500.yaml`); Lote 3 executado (3 automações de movimento desabilitadas e estruturalmente mortas removidas de `automations.yaml`: `Banheiro - Iniciou Movimento`, `Banheiro - Parou Movimento`, `Cozinha - Iniciou Movimento` — gatilho e alvo de ação inexistentes, sem consumidor); `packages/_disabled/*` e `lovelace.teste_4` já resolvidos sem ação necessária; `Lab - Muda o Tema Automaticamente`, `Coloca o tema Claro`, `Corredor - Parou/Iniciou Movimento` confirmados em uso real (disparo comprovado nas últimas 24h) e preservados. PEND-012 permanece ABERTA porque a própria `docs/auditoria_legado_v20_1c.md` ainda lista saldo real não qualificado individualmente: restante de Quarentena C1 (inclui `Cozinha - Parou Movimento`, ainda não avaliado), C2, C3 e Bloqueado, cada um reservado para lote formal próprio pela estratégia de execução futura da auditoria |
