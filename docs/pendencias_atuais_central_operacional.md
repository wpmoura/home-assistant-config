...
| PEND-012 | V20.1C/decommission | SOC | Definir e autorizar lotes pequenos de desativação com rollback | decisão | Sim para qualquer remoção | ABERTA | Quarentena C1 de `docs/auditoria_legado_v20_1c.md` **integralmente qualificada** (35/35 itens com decisão): 8 removidos nos Lotes 1-3 + 3 no Lote 4 + 3 scripts (`inicio_do_trabalho`, `home_office_iniciar_expediente_strip_light`, `desliga_tudo`) e 4 blueprints órfãos (`iluminacao_presenca_piso.yaml`, `Liga Luz do Quarto com Circadiano (Avançado).yaml`, `wmoura - Ligar Luz - Quarto - Circadiano.yaml`, `desligar_luz_quarto.yaml`) removidos no Lote 5 — total 18 objetos removidos; 21 itens preservados (uso real comprovado); `Homeoffice - Ligar Regua USB` e `Home office - Desliga Regua USB` seguem **desabilitadas via `automation.turn_off` (runtime), em observação operacional**, decisão de remover/reativar pendente de Gate futuro; 2 itens (`Lab - Liga/Desligar input_boolean.hd_backup`) já resolvidos pela PEND-018. Nenhum item de C1 permanece sem classificação. PEND-012 permanece ABERTA — próxima decisão deve considerar saldo REAL acionável em Quarentena C2, C3 e Bloqueado (não a mera existência histórica dessas categorias), e a decisão final sobre a Régua USB em observação |
