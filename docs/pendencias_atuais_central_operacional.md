...
| PEND-012 | V20.1C/decommission | SOC | Definir e autorizar lotes pequenos de desativação com rollback | decisão | Sim para qualquer remoção | ABERTA | Quarentena C1 integralmente qualificada e executada (Lotes 1-5, 18 objetos removidos). Triagem de C2/C3/Bloqueado (48 itens) qualificou saldo removível de 10 itens; Lote 6 executou 7 deles: 5 scripts mortos de `scripts_erro500.yaml` (`teste`/"Let's rock", `1691176415030`/"Good bye", `1743352611708`/"Anunciar via Alexa", `forcar_modo_dormir`, `teste_botao_acordar`) e 2 automações desabilitadas de `automations.yaml` (`Lab - Notificar quando downloads forem concluídos_WM`, `Quarto - Notificação intensidade de Chuva`). **NO-GO individual** para `LAB - Notificar Início do Home Assistant - WM` — habilitada, ação funcional específica, sem substituta comprovada, `last_triggered: null` isoladamente insuficiente; preservada. C3 (10 itens) e Bloqueado (13 itens) classificados PRESERVAR/NÃO ACIONÁVEIS — nenhuma dívida de decommission real neles. Saldo restante explícito: `Lab - Moeda - Dólar/Euro abaixo de...` (candidatas a desabilitar-e-observar, ~3,4 anos sem disparo, não decidido) e `Homeoffice - Ligar/Desliga Regua USB` (em observação operacional desde o Lote 4). PEND-012 permanece ABERTA por esse saldo explícito remanescente, não por categorias históricas |
