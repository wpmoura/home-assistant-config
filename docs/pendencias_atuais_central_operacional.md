...
| PEND-012 | V20.1C/decommission | SOC | Definir e autorizar lotes pequenos de desativação com rollback | decisão | Sim para qualquer remoção | ABERTA | Lote 1 executado (`automation.sala_porta_da_sala_abriu`; blueprint `wan_4G_AppleWatch.yaml` sem `_Energia`); Lote 2 executado (3 scripts mortos removidos de `scripts_erro500.yaml`); Lote 3 executado (3 automações de movimento removidas de `automations.yaml`); Lote 4 executado — `Cozinha - Parou Movimento` e `Desliga everything` removidas (já desabilitadas, gatilho/ação inexistentes); `Homeoffice - Ligar Regua USB` e `Home office - Desliga Regua USB` **desabilitadas via `automation.turn_off` (runtime, não removidas do YAML)** e colocadas em observação operacional — uso real do switch associado (`switch.regua_zigbee_br_l5`) não corresponde ao disparo destas automações; decisão de remover/preservar fica para Gate futuro; `Home office - Baixar Mesa` preservada (disparo recente, dashboard de produção). `packages/_disabled/*` e `lovelace.teste_4` já resolvidos sem ação necessária. PEND-012 permanece ABERTA porque a própria `docs/auditoria_legado_v20_1c.md` ainda lista saldo real não qualificado individualmente: restante de Quarentena C1 (a partir de `Home office - Cena Memória 4`), C2, C3 e Bloqueado, cada um reservado para lote formal próprio |
