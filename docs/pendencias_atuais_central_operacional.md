...
| PEND-012 | V20.1C/decommission | SOC | Definir e autorizar lotes pequenos de desativação com rollback | decisão | Sim para qualquer remoção | ABERTA | Lote 1 executado (`automation.sala_porta_da_sala_abriu`; blueprint `wan_4G_AppleWatch.yaml` sem `_Energia`); Lote 2 executado (3 scripts mortos removidos de `scripts_erro500.yaml`: `verifica_subida_do_sistema_ha`, `resetar_nivel_do_umidificador`, `1743721090083`); `packages/_disabled/*` e `lovelace.teste_4` já resolvidos sem ação necessária; `Lab - Muda o Tema Automaticamente` e `Coloca o tema Claro` confirmados em uso real (disparo diário comprovado) e não são mais candidatos. PEND-012 permanece ABERTA porque a própria `docs/auditoria_legado_v20_1c.md` ainda lista saldo real não qualificado individualmente: restante de Quarentena C1, C2, C3 e Bloqueado, cada um reservado para lote formal próprio pela estratégia de execução futura da auditoria — não avaliado nesta rodada |
