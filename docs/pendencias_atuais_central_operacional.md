...
| PEND-021 | V20.1C/legado — blueprint Bloqueado (monitoramento de energia crítica) | SOC | Duas automações órfãs `v5` removidas do YAML e da configuração carregada; registro residual no `entity_registry` e calibração de corrente da cervejeira permanecem como resíduos separados | correção de duplicidade morta + calibração separada | Não — os equipamentos permanecem protegidos pelas automações `v6`, nunca tocadas; limpeza de registry e calibração de corrente são resíduos independentes e não bloqueiam nada | ABERTA | **Gate P2 delimitado executado e validado em 2026-09-16, com PASS operacional.** Correção ao achado anterior (que descrevia as `v5` como "em operação normal"): confirmado que existem quatro automações relacionadas — duas órfãs `use_blueprint: wmoura/monitoramento_energia_v5.yaml` (caminho inexistente em Git e na listagem live de blueprints, nunca versionado neste repositório) e duas irmãs funcionais `use_blueprint: wmoura/monitoramento_energia_v6.yaml`. Backup do `automations.yaml` operacional (`/Volumes/config`) criado e conferido por sha256 antes de qualquer edição. Removidas nele **somente** as duas entradas `v5` — `id 1777423497558` (Geladeira) e `id 1777424244998` (Cervejeira); nenhum outro ID, parâmetro ou arquivo operacional foi tocado, confirmado por diff completo contra o backup e por `git status` mostrando as demais alterações locais preexistentes de `/Volumes/config` inalteradas. `check_config` PASS antes do reload; `automation.reload` (isolado, sem restart do Core) executado com sucesso. **Pós-reload, confirmado sem presumir:** as duas `v6` — `Cozinha - Gestão energia Geladeira` (`id 1777424726293`) e `Dispensa - Gestão Energia Cervejeira` (`id 1777425009523`) — seguem `state: on`, com todos os parâmetros idênticos, incluindo `limite_corrente_alta: 3` da cervejeira, intocado; as duas `v5` saíram da configuração carregada, mas seus registros **permanecem no `entity_registry`** como `unavailable`/`restored: true` — a remoção do YAML não os limpou, como já esperado, e nenhuma limpeza de registry foi feita nesta janela. Nenhuma regressão observada na proteção dos equipamentos; rollback não foi necessário. **Resíduos remanescentes, não bloqueantes:** (1) limpeza dos dois registros órfãos (`automation.nova_automacaogeladeira_2`, `automation.dispensa_gestao_energia_cervejeira`) do `entity_registry`, mesmo padrão já usado em PEND-005/PEND-018, ainda não autorizada nesta janela; (2) calibração do `limite_corrente_alta=3` da cervejeira — Wilson não recorda a origem do valor (divergente do `5.5` que a `v5` morta tinha) — valor atual preservado sem alteração, decisão separada e independente, não bloqueante |
