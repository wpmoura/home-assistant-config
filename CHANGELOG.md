...
## [health-check-timeline-mensal-coerencia-temporal] - 2026-09-21

- **Health Check → Timeline V20 (homologado, 2026-09-19).** Nova automação `saude_sistema_health_check_publicar_timeline_v20` (`packages/saude_sistema_timeline.yaml`) reage ao evento `health_check_state_changed` (nunca ao estado do sensor, que volta de `unavailable` a `success` a cada restart) e publica pelo publicador canônico apenas o resultado terminal de execução REAL (`origem` manual ou scheduled, `modelo` diferente de `mock`): `source: health_check`, `success` → `health_check_completed` ("✅ Health Check concluído"), `failed` → `health_check_failed` ("❌ Health Check falhou"). `interrupted`, estados intermediários, MOCK e `system_test` não publicam. `request_id`/`session_id` derivam do `execution_id`. Allowlist acrescida em `contrato_publicacao_timeline_v20.yaml` e na cópia do motor, com exceção de dedup por texto restrita a esses dois eventos (10 pontos `heartbeat_excecao`); heartbeat, `ha_started`, SmallTV e demais produtores inalterados. Evidência: execução manual real `hc-mu8v8vxy-jaj2pllz`, ACK `published`, uma única entrada `✅ Health Check concluído` na Timeline, trace da automação coerente.
- **Frequências mensais (implantadas e validadas, 2026-09-21).** `input_select.saude_sistema_health_check_frequencia` passa a oferecer `1x por mês` (dia 1) e `2x por mês` (dias 1 e 15), sempre às 08:00 America/Sao_Paulo; as cinco opções anteriores e o `initial: Desativado` ficaram inalterados. A lógica do scheduler (`gate53d_fn_decidir`, tab `gate53b_tab` do Node-RED) foi implantada diretamente no runtime por um único `PUT /flow/gate53b_tab` (somente o campo `func` desse nó) e **não é versionada neste repositório**. Hash SHA-256 canônico da tab: baseline/rollback `45156a39c397087e745f4874c649d9aecca18c5558fd765fc7425a029f6a4ed2`; pós-PUT `bfd6ffd65fd62f0775ee58cecb1bd32f101d521819a752c4e6d7476e6ac09ba5`. Regressão offline: 15.330 combinações das opções existentes sem divergência, mais 17 casos novos conformes. A frequência permanece `Desativado`; nenhuma execução real numa janela mensal ocorreu ainda (primeira janela natural: 01/10/2026 às 08:00, condicionada a selecionar uma das novas opções).
- **Coerência temporal da página Saúde do Sistema.** O dashboard `sistema-casa` (view `saude-do-sistema`) foi atualizado pela API suportada do HA (`ha_config_set_dashboard`, com trava de hash): 12 campos (11 conteúdos e 1 título) sem alterar lógica. `.storage` é Lovelace Storage e **não é versionado** (`.gitignore`); o dashboard não faz parte do Git. Os cinco blocos determinísticos (farol, contadores, domínios, ação, alertas) continuam sendo **publicação manual** (`sensor.saude_sistema_status`, `source: claude_hamcp_manual`, último `checked_at` 2026-09-01T01:01:25-03:00; nenhum produtor automático foi criado) e agora são identificados como tal, com data da publicação e frescor do watchdog; "O que preciso fazer hoje?" virou "Ação sugerida pelo último diagnóstico". Estado, Resultado e Telemetria passaram a distinguir última execução, execução em andamento e falha após sucesso, e os rótulos obsoletos (`(MOCK)` em `calling`, "Custo real acumulado", texto de homologação do ramo MOCK) foram corrigidos.
- **Metadados.** `contrato.publicado_por` de `sensor.saude_sistema_analitico_status` e o comentário obsoleto "Ainda 100% MOCK" do `input_button` foram corrigidos. O atributo já está implantado (`template.reload` em 2026-09-21) e será materializado naturalmente no próximo evento legítimo do Health Check, pois o sensor é trigger-based e restaura seus atributos até lá.
- Fora desta frente: a procedência de `last_changed` na coleta de evidências enviada à IA (o valor reinicia no boot do HA e a análise pode afirmar durações menores que a real) não foi tratada.
